...
sort : List -> List
sort = synth ($\lambda$ xs r ->  isIncreasing r && isPermutation xs r)

isIncreasing : List -> Bool
isIncreasing xs = and (zipWith (<=) xs (drop 1 xs))
...
We have big-step semantics given by the relation $\cdot \seq \cdot \bigs_{\cdot} \cdot \toCtx \cdot$ which is a subset of $Ctx \times Expr \times Name \times SSL \times Ctx$ satisfying. This is defined using some other relations:
...
\[
  \begin{array}{c}
    \infer{\Gamma \seq \lst{(case lift @L l of | C1 a -> a | C2 a b -> a + b)}
      \bigs_r
        - \toCtx -}
      {\infer{- \seq \lst{lift @L l} \Lbigs_s - \toCtx -}{}
         & \infer{- \seq \lst{C1 a -> a} \Abigs_r y_1 \toCtx -}{}
         & \infer{- \seq \lst{C2 a b -> a + b} \Abigs_r y_2 \toCtx -}{}}
  \end{array}
\]

...
\[
  \begin{array}{c}
    ...
    \labinfer{\Gamma \seq \lowerS \layoutPart{@(L\; (C\; \overline{p}) (C\; \overline{v}) := L_2(\overline{z}) \sep \kappa)}
      \Lbigs_n
    ...
    \labinfer{\Gamma \seq \lowerS \layoutPart{@(L\; (C\; \overline{p}) (C\; \overline{v}) := \ttt{emp})}
      \Lbigs_n
        \ttt{skip} \toCtx \Gamma}
      {}
...
\[
  \begin{array}{c}
    \infer{\Gamma \seq \Tlst{(case lift @L l of | C1 a -> a | C2 a b -> a + b)}
      \bigs_r
        - \toCtx -}
      {\infer{- \seq \Tlst{lift @L l} \Lbigs_s - \toCtx -}{}
         & \infer{- \seq \Tlst{C1 a -> a} \Abigs_r y_1 \toCtx -}{}
         & \infer{- \seq \Tlst{C2 a b -> a + b} \Abigs_r y_2 \toCtx -}{}}
  \end{array}
\]
...
inputs and outputs. For example, in the specification of \ttt{sort} from a previous section, we have the logical predicate
...
If we fix the first argument to some list value (say \lst{sortP someList : List -> Bool}) and we consider the resulting predicate on the "result values", we have
...
A value has this type if the value is in the \textit{preimage} of \lst{True} under \lst{sortP someList}. Equivalently, the following diagram
...
\noindent
Considering all possible inputs, we find that a valid synthesis result is a function of type \lstinline{(a : A) $\ra$ Valid(p, a)}.

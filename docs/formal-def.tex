...
\[
  \begin{array}{c}
    \labinfer{\ttt{instantiate}_{L{_1},L_{2}}\sem{f\; (C_i\; \overline{a}) | e_b := e} = f(\Delta) : (h_b \land C_b) \Ra h \sep h_e}
      {\begin{gathered}
         \Sigma \cap \overline{a} = \varnothing
             ~~~ \Gamma \vdash (\Sigma \rhd \lowerS{L_1}\sem{C_i\; \overline{a}} \bigs h) ~~~
         \Gamma, \overline{a} \vdash (\Sigma \rhd \lowerS{L_2}\sem{e} \bigs h_e)\\
         \Gamma \vdash (\Sigma \rhd \mathscr{C}_{\Sigma}\sem{C_i} \bigs C_b) ~~~
...
Every subexpression must have a type that's "closed" in the sense that it doesn't contain any (constrained) "layout type variables" (like \ttt{List}). Maybe

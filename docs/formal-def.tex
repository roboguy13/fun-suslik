...
We have big-step semantics given by the relation $\cdot \seq \cdot \bigs_{\cdot} \cdot \toCtx \cdot$ which is a subset of $Ctx \times Expr \times Name \times SSL \times Ctx$ satisfying:
...
\[
  \begin{array}{c}
    ...
    \labinfer{\Gamma \seq \lowerS @L\; (C\; \overline{v})
        \bigs_r x \toCtx \Gamma'}
      {\Gamma \seq \lowerS @L\; (C\; \overline{v}) \Lbigs_{L,r} x \toCtx \Gamma'}
...
\[
  \begin{array}{c}
    \labinfer{\Gamma \seq \lowerS \layoutPart{@(L\; (C\; \overline{p}) := [n, \iota] \sep \kappa)} (C\; \overline{v}) 
       \Lbigs_n
       \ttt{let $n$ $\la$ malloc}(\iota) ; y \toCtx \Gamma'}
    ...
    \labinfer{\Gamma \seq \lowerS \layoutPart{@(L\; (C\; \overline{p}) := (x+\iota) \pointsto a \sep \kappa)} (C\; \overline{v})
      \Lbigs_n
        \ttt{*}(n+\iota) \la v_{\iota}; y \toCtx \Gamma'}
      {\Gamma \seq \lowerS \layoutPart{@(L\; (C\; \overline{p}) := \kappa)} (C\; \overline{v}) \Lbigs_n y \toCtx \Gamma'}
    ...
    \labinfer{\Gamma \seq \lowerS \layoutPart{@(L\; (C\; \overline{p}) (C\; \overline{v}) := emp)}
      \Lbigs_n
        \ttt{skip} \toCtx \Gamma}
      {}
...
% \[
%   \begin{array}{c}
%     \labinfer{(case lift @L l of | C1 a -> a | C2 a b -> a + b)
%       \bigs_r
%         (if 
%   \end{array}
% \]

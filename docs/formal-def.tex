...
What layout does this correspond to? Starting with the outermost \ttt{Cons}:
...
$\cdot; \cdot\Lbigs\cdot \subset \LayoutCase \times \FsValue \times \Heap$ which is defined by

\[
  \begin{array}{c}
    \infer{(\lambda (C_i[z;\overline{w}]\; \overline{p}) \ra \emp); (C_i\; \overline{a}) \Lbigs x = 0}{\overline{a \val}}
    \\\\
    \infer{(\lambda (C_i[x;\overline{y}]\; \overline{p}) \ra x \pointsto z, \kappa); (C_i\; \overline{a}) \Lbigs x \pointsto z \sep \kappa'}
      {(\lambda (C_i[x;\overline{y}]\; \overline{p}) \ra \kappa); (C_i[x;\overline{y}]\; \overline{a}) \Lbigs \kappa' & \overline{a \val}}
    \\\\
    \infer{(\lambda (C_i\; \overline{p}) \ra L[\overline{x}] (C_j[x;\overline{y}] \overline{b}), \kappa); (C_i[x;\overline{y}]\; \overline{a})
            \Lbigs \kappa_L \sep \kappa'}
      {
      & (\lambda (C_j[z;\overline{w}]\; \overline{q}) \ra h) \in \Sigma(L[\overline{x}] (\overline{b})) \\
\overline{a \val}
      & (\lambda (C_j[z;\overline{w}]\; \overline{q}) \ra h); (C_j[x;\overline{y}]\; \overline{b}) \Lbigs \kappa_L
 & \overline{b \val} \\
      & (\lambda (C_i[x;\overline{y}]\; \overline{p}) \ra \kappa); (C_i[x;\overline{y}]\; \overline{a}) \Lbigs \kappa'
      }
  \end{array}
\]
...
\begin{lemma}
  The relation $\cdot \vdash \cdot\Lbigs\cdot$ defines a unique function $(\Ctx \times \FsExpr \times (D \monic \layout{\overline{x}})) \ra \Heap$:\\
  $\forall \Sigma, e, L, h, h'. (\Sigma \vdash e \Lbigs h) \land (\Sigma \vdash e \Lbigs h') \implies h = h'$
\end{lemma}
...
\[
  \begin{array}{c}
    ...
    \\\\
    \infer{\Sigma \vdash e \Hbigs r \pointsto x \sep x \pointsto i \sep \overline{(x+j) \pointsto v_j}}{\Sigma ; \cdot \vdash e \bigs\; C_i\; \overline{v_j}}
    \\\\
    \infer{\Sigma \vdash e \Hbigs h}{\Sigma \vdash e \Hbigs \lowerS{L} (C_i\; \overline{v_j}) & \Sigma \vdash (C_i\; \overline{v_j}) \Lbigs h}
  \end{array}
\]

\noindent
\textbf{Note}: This semantic function is only defined for \textit{closed} fun-SuSLik terms.

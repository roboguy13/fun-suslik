...
The relation for instantiating layouts with specific ADT values is
$\cdot \vdash \cdot\Lbigs{\cdot}\cdot \subset \Ctx \times \FsExpr \times (D \monic \layout{\overline{x}}) \times \Heap$ which is defined by

\[
  \begin{array}{c}
    \infer{\Sigma \vdash (C_i\; \overline{x}) \Lbigs{L} \TranslateLayout{d}[p:=x]}{\Sigma \ni (L (C_i\; \overline{p}) := d)}
  \end{array}
\]
...
define a semantic relation $\Hbigs\; \subset \FsExpr \times \Heap$ using $\bigs$ as follows:

\[
  \begin{array}{c}
    \infer{e \Hbigs r \pointsto i}{\Sigma ; \cdot \vdash e \bigs\; i & i \in \mathbb{Z}}
    ~~~
    \infer{e \Hbigs r \pointsto b}{\Sigma ; \cdot \vdash e \bigs\; b & b \in \mathbb{B}}
    \\\\
    \infer{e \Hbigs r \pointsto x \sep x \pointsto i \sep \overline{(x+j) \pointsto v_j}}{\Sigma ; \cdot \vdash e \bigs\; C_i\; \overline{v_j}}
    \\\\
    \infer{e \Hbigs h}{e \Hbigs \lowerS{L} (C_i\; \overline{v_j}) & (C_i\; \overline{v_j}) \Lbigs{L} h}
  \end{array}
\]
...
\[
  \begin{array}{c}
    \infer{\Gamma \seq \ttt{(case lift @L l of | C1 a $\ra$ a | C2 a b $\ra$ a + b)}
      \bigs_r
        - \toCtx -}
      {\infer{- \seq \ttt{lift @L l} \Lbigs{L}s - \toCtx -}{}
...
\begin{theorem} Completeness. \textbf{TODO}: Is this actually completeness?\\
  $\forall e, P, Q.\; \Fsem{e} = (P, Q) \Ra \forall x.\; \Hsem{x} \models P \Ra \Hsem{e(x)} \models Q$
\end{theorem}

\begin{theorem} Soundness.\\
  $\forall P, Q. \forall e : A \ra B, x : A.\; \Fsem{e} = (P, Q) \Ra\; \vdash e(x) : B \Ra \Hsem{x} \models P$
\end{theorem}

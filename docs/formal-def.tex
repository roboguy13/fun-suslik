...
\begin{math}
  \sem{\ttt{data T := }\overline{\ttt{alts}_i}\ttt{;}} = p^{\alpha}(x, \Nsem{0}{\,\overline{\ttt{alts}_i}\,}) : \overline{\Asem{i}{\ttt{alts}_i}}
  \\
  \\
  ...
  \\
  \\
  \Asem{i}{\ttt{C }\overline{\ttt{fields}_j}\,} = \ttt{(which == i)} \Ra \ttt{[x, j+1]} * x \mapsto \ttt{which} * \overline{\Fsem{i+1,j}{\ttt{fields}_j}}
  \\
  \\
  \Fsem{i,j}{\Int} = (x+i) \mapsto v_{i+j}
  \\
  \Fsem{i,j}{\Bool} = (x+i) \mapsto v_{i+j}
  \\
  \Fsem{i,j}{\ttt{T } \overline{\ttt{args}_k}} = (x+i) \mapsto v_{i+j} * \ttt{T}^{\alpha}(v_{i+j}, \overline{\ttt{args}_k})
\end{math}
\\

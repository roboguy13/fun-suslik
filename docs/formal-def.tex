...
We have big-step semantics given by the relation $\cdot \seq \cdot \bigs_{\cdot} \cdot \toCtx \cdot$ which is a subset of $Ctx \times Expr \times Name \times SSL \times Ctx$:
...
\[
  \begin{array}{c}
    ...
    \labinfer{\Gamma \seq \ttt{specialize}\; \overline{@L}\; @L_0\; (\lambda \overline{p}\ra e)\; \overline{x}
      ...
      % \}
      }{\Gamma \seq \overline{\lowerS_L x \bigs_v x'} \toCtx \Gamma' & \Gamma' \seq \lowerS_{L_0} e \bigs_r e' \toCtx \Gamma''}
...
\[
  \begin{array}{c}
    \labinfer{\Gamma \seq \lowerS \layoutPart{@(L\; (C\; \overline{p}) := [n, \iota] \sep \kappa)} (C\; \overline{v}) 
       \Lbigs_n
       \lst{let n <- malloc}(\iota) ; y \toCtx \Gamma'}

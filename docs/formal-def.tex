...
\begin{grammar}
  ...
  <e$_0$> ::= <I> | <B> | <L> | <C> | \letbnd\; x \ttt{:=} <e> \inexpr\; <e> | ( <e> )

  <L> ::= $\lambda x \ra$ <e>

  <C> ::= \ttt{convert}$_{\angled{T},\angled{T}}$ | \fold{\langle T\rangle} | \ite\; | \lesseq\; | \eq\; | \add\; | \sub\; | \ttt{and}\; | \ttt{or}\; | \ttt{not}

  <T> ::= \ttt{Int} | \ttt{Bool} | <T> $\ra$ <T> | ( <T> )

    ...
    \labinfer{\ttt{gen}_{L{_1},L_{2}}\sem{f\; (C\; \overline{a}) | e_b := e} = f(\Delta) : (h_b \land C_b) \Ra h \sep h_e}
      {\begin{gathered}
         \Sigma \cap \overline{a} = \varnothing
             ~~~ \Gamma \vdash (\Sigma \rhd \lowerS{L_1}\sem{C\; \overline{a}} \bigs h) ~~~
         \Gamma, \overline{a} \vdash (\Sigma \rhd \lowerS{L_2}\sem{e} \bigs h_e)\\
         \Gamma \vdash (\Sigma \rhd \mathscr{C}_{\Sigma}\sem{C} \bigs C_b) ~~~
         \Gamma \vdash (\Sigma \rhd \Esem{e_b} \bigs h_b)
         \\
         \Gamma \vdash f : \alpha \ra \beta ~~~
         \overline{\Gamma \vdash C : \tau_{1} \ra \tau_{2} \ra \cdots \ra \alpha}
         \\
...
\begin{equation*}
  \Sigma \rhd \mathscr{C}\sem{C} \bigs b
\end{equation*}
...
\[
  \begin{array}{c}
    \labinfer{\Gamma \vdash (\Sigma \rhd \mathscr{C}\sem{C} \bigs h_b)}{(L (C\; \overline{a}) := h) \in \Sigma & h_b = \bigwedge S_\Sigma\sem{C}(h)}
    % \\\\
...
\[
  \begin{array}{c}
    \labinfer{\Gamma \vdash (\Sigma \rhd \Esem{x} \bigs x)}{x \in \Var & x \in \Gamma}
    ~~~
    \labinfer{\Gamma \vdash (\Sigma \rhd \Esem{i} \bigs i)}{i \in \mathbb{Z}}
    ~~~
    \labinfer{\Gamma \vdash (\Sigma \rhd \Esem{b} \bigs b)}{b \in \mathbb{B}}
    \\\\
    \labinfer{\Gamma \vdash (\Sigma \rhd \Esem{e_1 + e_2} \bigs (s_1 + s_2))}{\Gamma \vdash (\Sigma \rhd e_1 \bigs s_1) & \Gamma \vdash (\Sigma \rhd e_2 \bigs s_2)}
    \\\\\
    \labinfer{\Gamma \vdash (\Sigma \rhd \Esem{\ttt{not}\; e} \bigs (\ttt{not}\; s))}{\Gamma \vdash (\Sigma \rhd e \bigs s)}
    \\\\
    \labinfer{\Gamma \vdash (\Sigma \rhd \Esem{e_1\; \ttt{\&\&}\; e_2} \bigs (s_1\; \ttt{\&\&}\; s_2))}{\Gamma \vdash (\Sigma \rhd e_1 \bigs s_1) & \Gamma \vdash (\Sigma \rhd e_2 \bigs s_2)}
  \end{array}
\]

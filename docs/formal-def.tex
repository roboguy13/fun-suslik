...
These are the four properties we consider:

\begin{enumerate}
  \item \itopic{Existence of Correct Programs}: For any fun-SuSLik expression, its corresponding layout function will exist in the set given by compiling it to SuSLik.
    ...
    \textbox{
    \proofidea: Given any fun-SuSLik expression $e$, construct a SuSLik derivation for $\compile(e)$ that
    has behavior $\sem{e}$.
  }
    ...
    %       & \mathcal{P}(Layout)
    %     \\ & Layout
    %       \arrow[rr, rightarrowtail, "x\mapsto\{x\}"]
    %     & & X
    %       % \arrow[u, rightarrowtail, "\rotatebox{180}{$\supseteq$}", sloped, swap]
    %       \arrow[u, rightarrowtail, swap]
    %       % \arrow[urr, rightarrowtail]
    % \end{tikzcd}

  \item \itopic{Compositionality}: Given two fun-SuSLik functions $f : A \ra B$ and $g : B \ra C$, we have

    % NOTE: This essentially gives a functor

    % \begin{equation*}
    %   s \in \Ssem{\compile(g \circ f)} \implies \exists q \in \Ssem{\compile(g)}, \exists p \in \Ssem{\compile(f)}, s = q \circ p
    % \end{equation*}

    \begin{equation*}
      \Ssem{\compile(g \circ f)} \subseteq \{ p \circ q \mid p \in \Ssem{\compile(g)} \land q \in \Ssem{\compile(f)} \}
    \end{equation*}

    \textbox{
      \proofidea: Given fun-SuSLik functions $f : A \ra B$ and $g : B \ra C$, and any a SuSLik derivation of $\Ssem{\compile(g \circ f)}$,
      show that the behavior of the resulting program is the same as the composition of some program from $\Ssem{\compile(g)}$ with some
      program from $\Ssem{\compile(f)}$.
    }

    % \begin{equation*}
    %   \Ssem{\compile(g \circ f)} = \Ssem{\compile(g) \circ \compile(f)}
    % \end{equation*}

  \item \itopic{Retraction}: $\compile$ and $\uncompile$ form a section/retraction pair. This means that we do not lose information when we uncompile something
    that we have compiled. Specifically, the $\compile$ transformation does not lose information.
    \begin{equation*}
      \uncompile \circ \compile = id
    \end{equation*}

    \textbox{
      \proofidea: We need to show that $\compile$ is an injective function, up to $\alpha$-equivalence.
    }
  ...
  % \item \itopic{Preservation of Restrictions}: If we are given any two fun-SuSLik functions $f, g : A \ra B$ where we define $X$ to
  %   be the set of inputs that the functions agree on $X = \{ a \med a \in A \land f(a) = g(a)$ and we define
  %   $h : X \ra B$ to be the restriction of $f$ (or, equivalently, $g$) to $X$, then we have
  %
  %   \begin{equation*}
  %     \Ssem{\compile(h)}(\Ssem{X}) 
  %   \end{equation*}

  \item \itopic{Lower Bound}: For any other translation $t : FsExpr \ra SuSLik$ that also satisfies the above properties, we have

    \begin{equation*}
      \forall e, |\Ssem{\compile(e)}| \le |\Ssem{t(e)}|
    \end{equation*}

  That is, the $\compile$ function gives the smallest set of possible
    synthesis results of any transformation that obeys the other properties.

  \textbox{
    \proofidea: Find any injective function $i : \Ssem{\compile(e)} \rightarrowtail \Ssem{t(e)}$. Alternatively,
  find any \textit{surjective} function $s : \Ssem{t(e)} \twoheadrightarrow \Ssem{\compile(e)}$.
  
  Also, note that there are specific synthesis rules which are non-deterministic such as \textsc{Close}. These will
  potentially (though not always) expand the space of programs that can be synthesis results from a given spec.
}
  ...
  %   $\tilde{t} : SuSLik \ra FsExpr$, we have an injective function
  %
  % \begin{equation*}
  %   i : \Ssem{\compile(e)} \rightarrowtail \Ssem{t(e)}
  % \end{equation*}

  % \begin{equation*}
  %   \forall e, \Ssem{\compile(e)} \subseteq \Ssem{t(e)}
  % \end{equation*}
  ...
  %     \arrow[ur, "t"]
  %     FsExpr
  %     \arrow[dr, "\compile", swap]
  %     \\ & SuSLik
  %       \arrow[r, "\Ssem{\cdot}"]
  %     & \mathcal{P}(Layout)
  %       \arrow[uu, rightarrowtail]
  % \end{tikzcd}

...
\begin{lemma}
  The relation $\cdot \vdash \cdot\Lbigs{\cdot}\cdot$ defines a unique function $(\Ctx \times \FsExpr \times (D \monic \layout{\overline{x}})) \ra \Heap$:\\
  $\forall \Sigma, e, L, h, h'. (\Sigma \vdash e \Lbigs{L} h) \land (\Sigma \vdash e \Lbigs{L} h') \implies h = h'$
\end{lemma}
...
\[
  \begin{array}{c}
    ...
    \\\\
    \infer{\Sigma \vdash e \Hbigs h}{\Sigma \vdash e \Hbigs \lowerS{L} (C_i\; \overline{v_j}) & \Sigma \vdash (C_i\; \overline{v_j}) \Lbigs{L} h}
  \end{array}
\]

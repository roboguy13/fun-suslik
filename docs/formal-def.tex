  ...
  \begin{array}{c}
    \labinfer{\ttt{gen}_{L{_1},L_{2}}\sem{f\; (C\; \overline{a}) | e_b := e} \longmapsto f(\Delta) : (h_b \land C_b) \Ra h \sep h_e}
      {\begin{gathered}
         ...
         \Sigma \rhd \mathscr{C}_{\Sigma}\sem{C} = C_b ~~~
...
\begin{equation*}
  \Sigma \rhd \mathscr{C}\sem{C} \longmapsto b
\end{equation*}
...
\[
  \begin{array}{c}
    \labinfer{\Gamma \vdash (\Sigma \rhd \mathscr{C}\sem{C} \longmapsto h_b)}{(L (C\; \overline{a}) := h) \in \Sigma & h_b = \bigwedge S_\Sigma\sem{C}(h)}
    % \\\\
